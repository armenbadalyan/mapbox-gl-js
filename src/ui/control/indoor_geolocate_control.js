...
function checkGeolocationSupport(callback) {
    if (supportsGeolocation !== undefined) {
        ...
        supportsGeolocation = window.IndoorAtlas !== undefined;
        callback(supportsGeolocation);
    }
}

/**
 * A `IndoorGeolocateControl` control provides a button that uses the IndoorAtlas
 * API to locate the user on the map.
 *
 *
 * The zoom level applied will depend on the accuracy of the geolocation provided by the device.
 *
 * The IndoorGeolocateControl has two modes. If `trackUserLocation` is `false` (default) the control acts as a button, which when pressed will set the map's camera to target the user location. If the user moves, the map won't update. This is most suited for the desktop. If `trackUserLocation` is `true` the control acts as a toggle button that when active the user's location is actively monitored for changes. In this mode the IndoorGeolocateControl has three states:
 ...
 *
 * @example
 * map.addControl(new mapboxgl.IndoorGeolocateControl({
 *     positionOptions: {
 ...
 * }));
 */
class IndoorGeolocateControl extends Evented {
    ...
    onRemove() {
        ...
        if (this._geolocationWatchID !== undefined) {
            window.IndoorAtlas.clearWatch(this._geolocationWatchID);
    ...
    _onClickGeolocate() {
        if (this.options.trackUserLocation) {
            ...
            if (this._watchState === 'OFF' && this._geolocationWatchID !== undefined) {
                ...
                this._geolocationWatchID = window.IndoorAtlas.watchPosition(
                    this._onSuccess, this._onError, this.options.positionOptions);
            }
        } else {
            window.IndoorAtlas.getCurrentPosition(
                this._onSuccess, this._onError, this.options.positionOptions);
 ...
 *
 * @event geolocate
 * @memberof IndoorGeolocateControl
 * @instance
