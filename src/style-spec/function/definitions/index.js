...
const expressions: { [string]: Class<LambdaExpression> } = {
    ...
    'curve': class extends LambdaExpression {
        static getName() { return 'curve'; }
        static getType() { return lambda(typename('T'), InterpolationType, NumberType, nargs(Infinity, NumberType, typename('T'))); }
        compile(args) {
            const interpolation = args[0].expression;
            if (!(interpolation instanceof LambdaExpression)) { throw new Error('Invalid interpolation type'); } // enforced by type checking

            const interpolationName = interpolation.constructor.getName();
            ...
            if (args[3].type === NumberType) {
                ...
                resultType = 'array';
            } else if (interpolationName !== 'step') {
                return {
                    errors: [`Type ${args[3].type.name} is not interpolatable, and thus cannot be used as a ${interpolationName} curve's output type.`]
                };
            }
            ...
            if (interpolation.name === 'exponential') {
                const baseExpr = interpolation.args[0];
    ...
    'step': class extends LambdaExpression {
        static getName() { return 'step'; }
        static getType() { return lambda(InterpolationType); }
        compile() {
            return { js: 'void 0' };
        }
    },
    'exponential': class extends LambdaExpression {
        static getName() { return 'exponential'; }
        static getType() { return lambda(InterpolationType, NumberType); }
        compile() {
            return { js: 'void 0' };
        }
    },
    'linear': class extends LambdaExpression {
        static getName() { return 'linear'; }
        static getType() { return lambda(InterpolationType); }
        compile() {
            return { js: 'void 0' };
        }
